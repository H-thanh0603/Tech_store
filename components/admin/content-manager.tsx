...
function DeleteButton({ kind, id }: { kind: 'banner' | 'section' | 'navigation'; id: string }) {
...
export function ContentManager(props: { banners: AdminBanner[]; sections: AdminHomepageSection[]; navigation: AdminNavigationItem[] }) {
  return <div className="space-y-5"><BannerPanel rows={props.banners} /><SectionPanel rows={props.sections} /><NavigationPanel rows={props.navigation} /></div>
}
...
function ItemTable({ rows, onEdit, kind }: { rows: Array<{ id: string; primary: string; secondary: string; active: boolean }>; onEdit: (id: string) => void; kind: 'banner' | 'section' | 'navigation' }) {
