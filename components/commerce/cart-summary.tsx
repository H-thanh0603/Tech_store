...
export function CartSummary({ cart }: CartSummaryProps) {
  ...
  return (
    <aside aria-labelledby="cart-summary-heading" className="rounded-(--radius-lg) border border-border bg-surface-raised p-5 shadow-sm">
      <h2 id="cart-summary-heading" className="text-(length:--text-xl) font-semibold text-fg">
        Tóm tắt đơn hàng
      </h2>
      <dl className="mt-5 flex flex-col gap-3 text-(length:--text-sm)">
        <div className="flex justify-between gap-4">
          <dt className="text-fg-muted">Tạm tính</dt>
          <dd className="font-medium text-fg">{formatPrice(cart.subtotal)}</dd>
        </div>
        <div className="flex justify-between gap-4">
          <dt className="text-fg-muted">Giảm giá</dt>
          <dd className="font-medium text-success">-{formatPrice(cart.discountTotal)}</dd>
        </div>
        <div className="flex justify-between gap-4">
        ...
        <div className="flex justify-between gap-4 border-t border-border pt-3 text-(length:--text-base)">
          ...
          <dd className="font-semibold text-fg">{formatPrice(cart.total)}</dd>
        </div>
      </dl>

      {isCheckoutDisabled ? (
        <p aria-live="polite" className="mt-4 text-(length:--text-sm) text-danger">
      ...
      {isCheckoutDisabled ? (
        ...
        <Link
          href="/checkout"
          className="mt-5 inline-flex min-h-11 w-full items-center justify-center rounded-md bg-accent px-4 text-sm font-medium text-accent-fg hover:bg-accent-hover"
        >
          Đến thanh toán
        </Link>
      )}
