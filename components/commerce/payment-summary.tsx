...
export function PaymentSummary({ order }: PaymentSummaryProps) {
  if (order.paymentMethod === 'cod') {
    return <p className="rounded-lg border border-border p-5">Thanh toán khi nhận hàng (COD).</p>
  }
  if (order.paymentStatus === 'expired' || order.orderStatus === 'expired') {
    return <p className="rounded-lg border border-danger p-5 text-danger">Phiên chuyển khoản đã hết hạn.</p>
  }
