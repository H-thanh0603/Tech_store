-- Allowlist the analytics-events throttle bucket (app/api/analytics/events).
-- Unknown actions return limited=true on first call, so the new bucket must
-- be listed here or every page view 429s. List mirrors
-- 202609140004_assistant_daily_buckets.sql + 'analytics_events'.
