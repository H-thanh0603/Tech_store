-- Agent layer (docs/AGENT_LAYER.md): the public read-only agent API
-- (/api/v1/agents/*) rate-limits via check_rate_limit. The RPC allowlist
-- blocks unknown buckets on the first call (returns true = limited), so the
-- new buckets must be allowlisted here or every agent request would 429.
