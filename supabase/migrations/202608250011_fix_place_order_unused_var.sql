-- Fix: db lint warning — variable "v_inventory" declared and select ... into,
-- but never read. Its only purpose was holding the row lock via FOR UPDATE.
-- Remove the declaration and use PERFORM ... FOR UPDATE to keep the lock.
...
begin
  ...
  -- ponytail: PERFORM holds the row lock without an unused variable.
  for v_item in
...
revoke all on function place_order(text, uuid, text, jsonb, text, text) from anon, authenticated;
