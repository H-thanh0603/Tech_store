-- Assistant daily cost-guard buckets: lib/assistant/rate-limit.ts checks
-- `${action}_daily` (assistant_chat_daily, merchant_chat_daily) after the
-- 15-minute burst bucket. The RPC allowlist blocks unknown buckets by
-- returning true (= limited), so without these entries EVERY assistant chat
-- request 429s with DAILY_LIMITED as soon as the DB is reachable. (When the
-- DB is down the route fail-opens, which is why this only bites live DBs.)
...
begin
  ...
  v_allowed := p_action in (
    ...
    'place_order',
    'assistant_chat', 'assistant_chat_daily',
    'merchant_chat', 'merchant_chat_daily',
