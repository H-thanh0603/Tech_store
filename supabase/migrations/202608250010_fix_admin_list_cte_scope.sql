-- Fix: PL/pgSQL CTEs (WITH) are statement-scoped, not function-scoped.
-- admin_list_orders and admin_list_customers declared CTEs (filtered, agg)
-- in a WITH clause consumed by one statement, then referenced them in a
-- *separate* SELECT statement — relation did not exist there (error 42P01).
-- Inline as WITH ... MATERIALIZED consumed by both count and row query in one statement.
...
begin
  ...
  select count(distinct full_count) into v_total from paged;

  select coalesce(jsonb_agg(to_jsonb(x) - 'full_count'), '[]'::jsonb)
  into v_rows
  from (
    select
